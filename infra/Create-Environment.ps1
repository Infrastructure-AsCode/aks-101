...
for ($i = 19; $i -le $InstanceCount; $i++) {
    ...
    az deployment sub create --location westeurope --template-file ./deployment.bicep  --parameters workloadName=$workloadName instanceId=$i sharedAcrName=$sharedAcrName -n "deployment-$i"
